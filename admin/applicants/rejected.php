...
<head>
    ...
    <title>Rejected Applicants | <?php echo $pages['page_website_title'];?></title>
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0">Rejected Applicants</h1>
                        </div><!-- /.col -->
                        <div class="col-sm-6">
            ...
            <section class="content">
                <form id="pen_app_form">
                    <div class="container-fluid">
                        <p>Note: Applicants that have been rejected more than 30 days will be automatically deleted. <a href="#" id="del_all" class="text-danger">Delete all rejected applicants</a></p>
                        <table id="tblDataTable" class="table table-hover">
                            ...
                            <tbody>
                                <?php
                                    $count = 1;
                                    $query=mySQLi_query($con, "SELECT * from tbl_applicants where appl_status = 'Rejected' order by appl_id DESC") or die(mySQLi_error($con));
        ...
        * Table Functions
        ...
        $("body").on('submit', '#pen_app_form', function(event){
            ...
            if($("input[name='appl_id[]']").is(":checked")){
                ...
                if(select_app_rej == 'Applicant'){title='Do you really want to move the selected applicants to pending?';toast_title='Successfully moved selected applicants to pending';btn='Move to pending'}
                else if(select_app_rej == 'Delete'){title='Do you really want to delete the selected applicants?';toast_title='Successfully deleted selected applicants';btn='Delete applicant/s'}
                Swal.fire({
                    title: title,
        ...
        $("body").on('click', '#del_all', function(event){
            event.preventDefault();
            if($("input[name='appl_id[]']").length > 0){
                Swal.fire({
                    title: 'Do you really want to delete all rejected applicants?',
                    text: "You won't be able to revert this!",
                    icon: 'question',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "php/pensioners_crud.php?d=1",
                            method: "GET",
                            processData: false,
                            contentType: false,
                            cache: false,
                            ...
                            success:function(response){
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Successfully deleted all rejected applicants',
                                    toast: true,
                                    position: 'top-end',
                                    showConfirmButton: false,
                                    timer: 3000,
