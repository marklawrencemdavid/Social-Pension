<?php
    ...
    if(isset($_SESSION['appl_data'])){
        ...
        // $contactNumber = $_SESSION['appl_data']['contactno'];
        ...
        if(mysqli_num_rows(mysqli_query($con, "SELECT appl_id FROM tbl_applicants WHERE appl_lastname = '".$lastname."', appl_firstname = '".$firstname."', appl_middlename = '".$middlename."', appl_houseno = '".$houseno."', appl_barangay = '".$barangay."'")) > 0){
            $appl_status='Spam';
        }
        ...
        if (mysqli_query($con, "INSERT into `tbl_applicants` (appl_lastname, appl_firstname, appl_middlename, appl_birthdate, appl_placeofbirth, appl_gender, appl_houseno, appl_barangay, appl_municipality, appl_province, appl_civilstatus, appl_prevoccupation, appl_contactno, appl_pensioner, appl_picture, appl_email, appl_status, appl_datesubmitted, appl_accesstoken, appl_proof)
            VALUES ('$lastname', '$firstname', '$middlename', '$birthdate', '$birthplace', '$gender', '$houseno', '$barangay', '$municipality', '$province', '$civilstatus', '$prevOccupation', '', '$pensioner', '$appl_picture', '$appl_email', '$appl_status', '".date('Y-m-d H:i:s')."', '".$code."', '".$appl_proof."')")) {
            // Data inserted id
            ...
            if(file_exists('../img/applicant_picture_temp/'.$appl_picture)){
                rename("../img/applicant_picture_temp/".$appl_picture, "../img/applicant_picture/".$appl_picture);
            }
            if(file_exists('../img/applicant_proof_temp/'.$appl_proof)){
                rename("../img/applicant_proof_temp/".$appl_proof, "../img/applicant_proof/".$appl_proof);
            }
            ...
            if ($err) {
                ...
                if(mysqli_query($con, "UPDATE tbl_applicants SET appl_accesstoken = '".$access_token."', appl_contactno = '".$subscriber_number."' WHERE  appl_id = '".$noti_data_id."'")){
                    echo 'Success!!<br>';
            ...
            if(file_exists('../img/applicant_picture/' . $appl_picture)){
                unlink('../img/applicant_picture/' . $appl_picture);
            }
